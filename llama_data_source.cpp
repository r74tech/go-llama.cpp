...
llama_file_source::llama_file_source(const char* filename) : fp(nullptr), file_size(0) {
...
size_t llama_file_source::tell() const {
    return ftell(fp);
}

size_t llama_file_source::size() const {
    return file_size;
}

bool llama_file_source::eof() const {
    return feof(fp) != 0;
}

// Memory buffer data source implementation
llama_memory_source::llama_memory_source(const void* buffer, size_t buffer_size)
    : data(static_cast<const uint8_t*>(buffer)), data_size(buffer_size), current_pos(0) {
}
...
size_t llama_memory_source::tell() const {
    return current_pos;
}

size_t llama_memory_source::size() const {
    return data_size;
}

bool llama_memory_source::eof() const {
    return current_pos >= data_size;
}
