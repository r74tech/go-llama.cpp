...
static std::vector<llama_token> tokenize_abi_safe(struct llama_context *ctx,
    ...
    n_tokens = llama_tokenize(ctx, text.c_str(), text.length(), result.data(),
                              result.size(), add_bos);

    if (n_tokens < 0) {
        result.resize(-n_tokens);
        n_tokens = llama_tokenize(ctx, text.c_str(), text.length(),
                                  result.data(), result.size(), add_bos);
    }
...
static std::string token_to_piece_abi_safe(const struct llama_context *ctx,
                                           llama_token token) {
    char buf[256];
...
static inline std::vector<llama_token>
tokenize_abi_safe(struct llama_context *ctx, const std::string &text,
                  bool add_bos) {
    return ::llama_tokenize(ctx, text, add_bos);
}

static inline std::string
token_to_piece_abi_safe(const struct llama_context *ctx, llama_token token) {
...
#define mkstemp(template)                                                      \
    _mktemp_s(template, strlen(template) + 1)                                  \
        ? -1                                                                   \
        : _sopen(template, _O_CREAT | _O_EXCL | _O_RDWR | _O_BINARY,           \
...
#include <unistd.h>
// Include sys/mman.h after defining _GNU_SOURCE to get memfd_create declaration
#include <sys/mman.h>
#ifndef MFD_CLOEXEC
...
llama_token llama_sample_token_binding(
    struct llama_context *ctx, struct llama_context *ctx_guidance,
    struct llama_grammar *grammar, void *params_ptr,
    const std::vector<llama_token> &last_tokens,
    std::vector<llama_token_data> &candidates, int idx = -1);
...
int get_token_embeddings(void *params_ptr, void *state_pr, int *tokens,
    ...
    for (int i = 0; i < tokenSize; i++) {
        ...
        std::string str_token(
            token_str); // create a new std::string from the char*
...
int eval(void *params_ptr, void *state_pr, char *text) {
    ...
    auto n_prompt_tokens = llama_tokenize(ctx, str.data(), str.length(),
...
int llama_predict(void *params_ptr, void *state_pr, char *result,
    ...
    if (params.n_ctx > 2048) {
        // TODO: determine the actual max context of the model (e.g. 4096 for
        // LLaMA v2) and use that instead of 2048
        fprintf(stderr,
                "%s: warning: base model only supports context sizes no "
                "greater than 2048 tokens (%d specified)\n",
    ...
    if (!path_session.empty()) {
        ...
        if (fp != NULL) {
            ...
            if (debug) {
                fprintf(stderr,
                        "%s: session file does not exist, will create\n",
                        __func__);
            }
        }
    }
    ...
    if (ctx_guidance) {
        guidance_inp = tokenize_abi_safe(ctx_guidance,
                                         params.cfg_negative_prompt, add_bos);
        std::vector<llama_token> original_inp =
            tokenize_abi_safe(ctx, params.prompt, add_bos);
    ...
    if (session_tokens.size() > 0) {
        ...
        if (debug) {
            if (params.prompt.empty() &&
                ...
                fprintf(stderr,
                        "%s: session file has exact match for prompt!\n",
                        __func__);
    ...
    if (debug && ctx_guidance) {
        ...
        for (int i = 0; i < (int)guidance_inp.size(); i++) {
            fprintf(stderr, "%6d -> '%s'\n", guidance_inp[i],
                    token_to_piece_abi_safe(ctx, guidance_inp[i]).c_str());
        }
    }
    ...
    if (!params.grammar.empty()) {
...

        {
            ...
            if (it != params.logit_bias.end() && it->second == -INFINITY) {
                fprintf(stderr,
                        "%s: warning: EOS token is disabled, which will cause "
    ...
    while (n_remain != 0) {
        ...
        if (embd.size() > 0) {
            ...
            if (n_past + (int)embd.size() + std::max<int>(0, guidance_offset) >
                ...
                embd.insert(embd.begin(),
                            last_tokens.begin() + n_ctx - n_left / 2 -
                                embd.size(),
            ...
            if (ctx_guidance) {
                ...
                if (n_past_guidance < (int)guidance_inp.size()) {
                    ...
                    // fprintf(stderr, "%s", llama_token_to_piece(ctx,
                    // embd_guidance[i]));
                    //}
                    // fprintf(stderr, "\n---------------------\n");
                } else {
                ...
                for (int i = 0; i < input_size; i += params.n_batch) {
                    ...
                    if (llama_eval(ctx_guidance, input_buf + i, n_eval,
                                   n_past_guidance, params.n_threads)) {
                        fprintf(stderr, "%s : failed to eval\n", __func__);
                        return 1;
                    }
        ...
        if ((int)embd_inp.size() <= n_consumed) {
            ...
            const llama_token id = llama_sample_token_binding(
                ctx, ctx_guidance, grammar, params_p, last_tokens, candidates);
            // const llama_token id = llama_sample_token(ctx, ctx_guidance,
            // grammar, params, last_tokens, candidates);
            ...
            // Create a mutable copy for the callback to avoid casting away
            // const
            std::string token_str_copy = token_str;
            if (!tokenCallback(state_pr,
                               const_cast<char *>(token_str_copy.c_str()))) {
                break;
            }
        } else {
...
// and be transparent to the caller, however this now maps 1:1 (mostly) the
// upstream implementation Note: both model have to be loaded with perplexity
// "true" to enable all logits
int speculative_sampling(void *params_ptr, void *target_model,
                         void *draft_model, char *result, size_t result_size,
                         bool debug) {

    gpt_params *params_p = (gpt_params *)params_ptr;
    llama_binding_state *target_model_state =
        (llama_binding_state *)target_model;
    ...
    while (true) {
        ...
        while (true) {
            ...
            // const llama_token id = llama_sample_token(ctx_tgt, NULL,
            // grammar_tgt, params, last_tokens, candidates, i_dft);
            const llama_token id =
                llama_sample_token_binding(ctx_tgt, NULL, grammar_tgt, params_p,
                                           last_tokens, candidates, i_dft);
            // remember which tokens were sampled - used for repetition
            // penalties during sampling
            last_tokens.erase(last_tokens.begin());
            last_tokens.push_back(id);
            ...
            if (i_dft < (int)drafted.size()) {
                LOG("the %dth drafted token (%d, '%s') does not match the "
                    "sampled target token (%d, '%s') - rejected\n",
                    i_dft, drafted[i_dft],
                    token_to_piece_abi_safe(ctx_dft, drafted[i_dft]).c_str(),
                    id, token_str.c_str());
            } else {
        ...
        for (int i = 0; i < n_draft; ++i) {
            ...
            // no need to evaluate the last drafted token, since we won't use
            // the result
            if (i == n_draft - 1) {
                break;
            }
...
int llama_tokenize_string(void *params_ptr, void *state_pr, int *result) {
    ...
    return llama_tokenize(ctx, params_p->prompt.data(),
                          params_p->prompt.length(), result, params_p->n_ctx,
                          add_bos);
}
...
int load_state(void *ctx, char *statefile, char *modes) {
...

    {
        ...
        llama_set_state_data(
            state,
            state_mem); // could also read directly from memory mapped file
        fclose(fp_read);
    }
...
void *load_model(const char *fname, int n_ctx, int n_seed, bool memory_f16,
                 ...
                 int n_gpu_layers, int n_batch, const char *maingpu,
                 const char *tensorsplit, bool numa, float rope_freq_base,
                 float rope_freq_scale, bool mul_mat_q, const char *lora,
                 const char *lora_base, bool perplexity) {
    return load_binding_model(
        fname, n_ctx, n_seed, memory_f16, mlock, embeddings, mmap, low_vram,
        n_gpu_layers, n_batch, maingpu, tensorsplit, numa, rope_freq_base,
        rope_freq_scale, mul_mat_q, lora, lora_base, perplexity);
}

void *load_binding_model_from_memory(
    const void *buffer, size_t buffer_size, int n_ctx, int n_seed,
    bool memory_f16, bool mlock, bool embeddings, bool mmap, bool low_vram,
    int n_gpu_layers, int n_batch, const char *maingpu, const char *tensorsplit,
    bool numa, float rope_freq_base, float rope_freq_scale, bool mul_mat_q,
    ...
    // The patch provides llama_load_model_from_buffer, so we can use it
    // directly
...
void *load_model_from_memory(const void *buffer, size_t buffer_size, int n_ctx,
                             ...
                             const char *lora_base, bool perplexity) {
    return load_binding_model_from_memory(
        buffer, buffer_size, n_ctx, n_seed, memory_f16, mlock, embeddings, mmap,
        low_vram, n_gpu_layers, n_batch, maingpu, tensorsplit, numa,
        rope_freq_base, rope_freq_scale, mul_mat_q, lora, lora_base,
        perplexity);
}
...
llama_token
llama_sample_token_binding(struct llama_context *ctx,
    ...
    llama_token_data_array cur_p = {candidates.data(), candidates.size(),
                                    false};

    if (ctx_guidance) {
        llama_sample_classifier_free_guidance(ctx, &cur_p, ctx_guidance,
                                              params.cfg_scale);
    }
    ...
    if (!last_tokens.empty()) {
        ...
        const int last_n_repeat =
            std::min(std::min((int)last_tokens.size(), repeat_last_n), n_ctx);

        llama_sample_repetition_penalty(ctx, &cur_p,
                                        last_tokens.data() +
                                            last_tokens.size() - last_n_repeat,
    ...
    if (temp <= 0) {
        ...
        if (mirostat == 1) {
            ...
            id = llama_sample_token_mirostat(ctx, &cur_p, mirostat_tau,
                                             mirostat_eta, mirostat_m,
                                             &mirostat_mu);
        } else if (mirostat == 2) {
            static float mirostat_mu = 2.0f * mirostat_tau;
            llama_sample_temperature(ctx, &cur_p, temp);
            id = llama_sample_token_mirostat_v2(ctx, &cur_p, mirostat_tau,
...
Currently we hard patch the following functions to common.cpp and common.h into
the llama library due to a bug into the nvcc/gcc compiler. It seems that copying
by value lead to a misalignment of structure and copy - resulting in a mixed up
values that we pass by.
...
void* load_binding_model(const char *fname, int n_ctx, int n_seed, bool
memory_f16, bool mlock, bool embeddings, bool mmap, bool low_vram, int
n_gpu_layers, int n_batch, const char *maingpu, const char *tensorsplit, bool
numa,  float rope_freq_base, float rope_freq_scale, bool mul_mat_q, const char
*lora, const char *lora_base, bool perplexity);
...
gpt_params* create_gpt_params(const std::string& fname,const std::string&
lora,const std::string& lora_base) { gpt_params* lparams = new gpt_params;
...
regex, -1}; std::vector<std::string> split_arg{it, {}};
            GGML_ASSERT(split_arg.size() <= LLAMA_MAX_DEVICES);
...
// Note: the only difference here is passing params as a pointer and avoid
copy-by-value
// We stick to another function to avoid patching all the llama.cpp code
// We need the function to be in the common.o object, as using it in the binding
does not make effect. llama_token llama_sample_token_binding( struct
llama_context * ctx, struct llama_context * ctx_guidance, struct llama_grammar *
grammar, const struct gpt_params * g_params,  // NOTE: this is our patch const
std::vector<llama_token> & last_tokens, std::vector<llama_token_data> &
candidates, int   idx) {
    ...
    const float   tfs_z           = params.tfs_z;
    const float   typical_p       = params.typical_p;
    const int32_t repeat_last_n   = params.repeat_last_n < 0 ? n_ctx :
params.repeat_last_n; const float   repeat_penalty  = params.repeat_penalty;
    const float   alpha_presence  = params.presence_penalty;
    const float   alpha_frequency = params.frequency_penalty;
    const int     mirostat        = params.mirostat;
    const float   mirostat_tau    = params.mirostat_tau;
    ...
    // Apply params.logit_bias map
    for (auto it = params.logit_bias.begin(); it != params.logit_bias.end();
it++) { logits[it->first] += it->second;
    }

    candidates.clear();
    for (llama_token token_id = 0; token_id < n_vocab; token_id++) {
        candidates.emplace_back(llama_token_data{token_id, logits[token_id],
0.0f});
    }
...
params.cfg_scale);
    ...
    if (!last_tokens.empty()) {
        const float nl_logit = logits[llama_token_nl(ctx)];
        const int last_n_repeat = std::min(std::min((int)last_tokens.size(),
repeat_last_n), n_ctx);
    ...
    if (temp <= 0) {
        ...
        if (mirostat == 1) {
            ...
            llama_sample_temperature(ctx, &cur_p, temp);
            id = llama_sample_token_mirostat(ctx, &cur_p, mirostat_tau,
mirostat_eta, mirostat_m, &mirostat_mu); } else if (mirostat == 2) { static
float mirostat_mu = 2.0f * mirostat_tau; llama_sample_temperature(ctx, &cur_p,
temp); id = llama_sample_token_mirostat_v2(ctx, &cur_p, mirostat_tau,
mirostat_eta, &mirostat_mu); } else {
            // Temperature sampling
            llama_sample_top_k      (ctx, &cur_p, top_k, 1);
            llama_sample_tail_free  (ctx, &cur_p, tfs_z, 1);
            llama_sample_typical    (ctx, &cur_p, typical_p, 1);
...

            {
                ...
                for (int i = 0; i < n_top; i++) {
                    ...
                    LOG(" - %5d: '%12s' (%.3f)\n", id,
token_to_piece_abi_safe(ctx, id).c_str(), cur_p.data[i].p);
                }
            }

            id = llama_sample_token(ctx, &cur_p);

            LOG("sampled token: %5d: '%s'\n", id, token_to_piece_abi_safe(ctx,
id).c_str());
        }
    }
